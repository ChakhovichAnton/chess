...
class ChessGameConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        clock_res = await self.game_has_ended_due_to_time()
    ...
    async def receive(self, text_data):
        ...
        if action == 'move':
            ...
            res = await self.game_has_ended_due_to_time()
        else:
    ...
    def surrender(self):
        try:
            game = ChessGame.objects.select_related('clock').get(id=self.game_id, status=ChessGame.GameStatus.ONGOING)
    ...
    @database_sync_to_async
    def game_has_ended_due_to_time(self):
        with transaction.atomic():
            try:
                game = ChessGame.objects.select_related('clock').get(id=self.game_id, status=ChessGame.GameStatus.ONGOING)
            except ChessGame.DoesNotExist:
